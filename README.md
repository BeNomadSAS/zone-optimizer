...
**A live Bemap web app** — draw a zone on the map, extract every road inside it, get an optimised turn-by-turn itinerary, and download a navigation-ready waypoint list.

## 🌐 Try it live

**→ [benomadsas.github.io/zone-optimizer](https://benomadsas.github.io/zone-optimizer/)**

You'll need a Bemap account to sign in (Beta / Preprod / Prod).

## Screenshot

Draw a polygon → click **Extract** → place **Start** and **End** → click **Optimise**. You get an optimised polyline, numbered waypoints, and a downloadable minimal-waypoint CSV ready for a nav hand-off.

## What it demonstrates

End-to-end Bemap integration in a handful of API calls:

| Capability | Bemap surface |
|---|---|
| Render the map (MapLibre + PMTiles) | `bemap.MapLibreMap` + `ctx.tilesHost` |
| Draw + edit a polygon | `bemap.MapLibreMap.drawPolygon` / `editPolygon` |
...
|---|---|---|
| Vector tiles (PMTiles) | `mptiles-api[-env].benomad.net` | Cookie (auto, via `bemap.Context`) |
...
**Option A — just try it**: go to [benomadsas.github.io/zone-optimizer](https://benomadsas.github.io/zone-optimizer/) and sign in with your Bemap account.

**Option B — self-host** the static bundle:
...
- **Environment** — Beta / Preprod / Prod (BeNomad hosts)
